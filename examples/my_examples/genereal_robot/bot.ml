  ...
  let main_step self ((time_21:float) , ()) =
    ((let (cindex_33:int) = cstate_32.cindex in
      ...
      (let (result_37:unit) =
           ...
           self.i_28 <- false ;
           (let (trigger_23:zero) = self.next_27.zin in
            let (l_26:float) = self.y_24.pos in
            (begin match trigger_23 with
                   | true ->
                       let () = print_endline (string_of_float l_26) in
                       let () = robot_str_ml ("transverse_vel") ((-30.)) in
                       let (next_31:int) = self.m_30 in
                       let (cpt_29:int) = (+) next_31  1 in
                       self.m_30 <- cpt_29 | _ -> ()  end) ;
            self.next_27.zout <- self.y_24.pos ;
       ...
       (if cstate_32.major then
        (((Zls.set cstate_32.cvec  !cpos_35  self.y'_25.pos ;
          ...
          (Zls.set cstate_32.cvec  !cpos_35  self.y_24.pos ;
           cpos_35 := (+) !cpos_35  1)) ; ((self.next_27.zin <- false)))
        else (((self.next_27.zin <- Zls.get_zin cstate_32.zinvec  !zpos_36 ;
                zpos_36 := (+) !zpos_36  1)) ;
              zpos_36 := zindex_34 ;
              ((Zls.set cstate_32.zoutvec  !zpos_36  self.next_27.zout ;
                zpos_36 := (+) !zpos_36  1)) ;
